...
function SignupService() {
  ...
  const handleSubmit = async (event) => {
    ...
    setError('');
    try {
      await signupUser({ ...form, role: 'service_provider' });
      connectSocket();
      navigate('/dashboard');
    } catch (apiError) {
  ...
  return (
    <AuthShell
      ...
      title="Create your service provider account."
      description="List your services and manage requests with a minimal workflow."
      footer={<p>Already have an account? <Link className="font-semibold text-[var(--accent-strong)]" to="/login/service-provider">Sign in here</Link></p>}
    >
      <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
        <input required value={form.name} onChange={(e) => setForm((current) => ({ ...current, name: e.target.value }))} className="field md:col-span-2" placeholder="Business or personal name" />

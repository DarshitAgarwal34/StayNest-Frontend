...
function SignupRenter() {
  ...
  const handleSubmit = async (event) => {
    ...
    setError('');
    try {
      await signupUser({ ...form, role: 'renter' });
      connectSocket();
      navigate('/dashboard');
    } catch (apiError) {
  ...
  return (
    <AuthShell
      ...
      description="Set up your renter profile and start managing properties with a cleaner flow."
      footer={<p>Already have an account? <Link className="font-semibold text-[var(--accent-strong)]" to="/login/renter">Sign in here</Link></p>}
    >
